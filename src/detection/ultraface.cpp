...
UltraFace::UltraFace(ts::TSQueue<std::unique_ptr<FrameInfo>>& input_queue_,
  ...
  ultraface.opt.use_vulkan_compute = false;
...
void UltraFace::infer() {
  ...
  while (true) {
    ...
    if (frame.empty() || frame.channels() != 3) {
      if (frame.channels() != 3) {
        std::cerr << "Error: Unexpected number of channels: "
...
void UltraFace::nms(std::vector<FaceInfo>& input, std::vector<FaceInfo>& output,
  ...
  for (int i = 0; i < box_num; i++) {
    ...
    switch (type) {
      ...
      default: {
        std::cerr << "Error: Invalid NMS type" << std::endl;
        exit(-1);
      }
    }
  }
